...
\maketitle
\begin{abstract}
The proliferation of large language models (LLMs) capable of generating syntactically correct and semantically coherent code has made it increasingly difficult to distinguish machine-generated code from human-written code---especially across different programming languages, application domains, and generation techniques. This paper presents the system of team \textbf{Procastic Simulators} for \textbf{SemEval-2026 Task~13: Detecting Machine-Generated Code with Multiple Programming Languages, Generators, and Application Scenarios}. We address all three subtasks: Subtask~A (binary detection of human vs.\ machine-generated code), Subtask~B (multi-class authorship attribution across 10 LLM families), and Subtask~C (hybrid code detection distinguishing fully human, fully machine, hybrid, and adversarial code). Starting from the provided CodeBERT baseline, we identify and fix six critical implementation bugs, then systematically improve Subtask~B performance through sqrt-scaled class-weighted cross-entropy loss, label smoothing, stratified subsampling to address the extreme class imbalance (88\% human-written), cosine learning rate scheduling, and rigorous hyperparameter search. For Subtask~A, we explore backbone alternatives (UniXcoder) and classification head architectures (Bi-LSTM, dense blocks with stylometric features). For Subtask~C, we have so far reproduced the official CodeBERT baseline for 4-class classification (human, machine, hybrid, adversarial), establishing reference performance on the validation set; targeted improvements are planned for subsequent phases. Our best Subtask~B configuration---CodeBERT with class-weighted loss, label smoothing ($\alpha{=}0.1$), and stratified subsampling---achieves a Macro F1 of 0.3676 after only 2 training epochs, improving substantially over the corrected baseline ($\sim$0.22). A separate UniXcoder experiment reaches Macro F1 of 0.470 after 5.7 epochs with a smaller human subset, demonstrating additional headroom. We present detailed per-class analysis, hyperparameter ablations, and a discussion of the challenges inherent in 11-class code authorship attribution under extreme imbalance.
\end{abstract}
...
The severe class imbalance in Subtask~B (88\% human) is the single largest obstacle to achieving competitive Macro F1 scores. Naive training with standard cross-entropy loss causes the model to predict the majority class (human) for nearly all inputs, achieving $\sim$88\% accuracy but near-zero F1 on all 10 LLM classes. We implement and evaluate the following strategies:

\begin{itemize}
    \item \textbf{Sqrt-Scaled Class-Weighted Loss}: We compute inverse-frequency class weights with square-root dampening (Equation~\ref{eq:class_weights}) and pass them to \texttt{torch.nn.CrossEntropyLoss} via a custom \texttt{WeightedTrainer} class that overrides the HuggingFace \texttt{Trainer.compute\_loss()} method. The square-root scaling prevents over-correction on extremely rare classes (e.g., BigCode and Gemma with only $\sim$2K samples).
    \begin{equation}
    w_c = \sqrt{\frac{N}{C \cdot n_c}}
    \label{eq:class_weights}
    \end{equation}
    where $N$ is the total number of (subsampled) training samples, $C = 11$ is the number of classes, and $n_c$ is the count of class~$c$.
    \item \textbf{Label Smoothing}: We replace hard one-hot targets with smoothed targets ($\alpha = 0.1$), preventing overconfidence on the dominant human class and improving probability calibration:
    \begin{equation}
    y'_c = (1 - \alpha)\,y_c + \frac{\alpha}{C}
    \end{equation}
    \item \textbf{Stratified Subsampling}: The human class is capped at a configurable \texttt{HUMAN\_SUBSET\_SIZE} (20K--40K), while all minority LLM classes are kept in full. This reduces the effective training set from 500K to $\sim$78K--98K samples, creating a substantially less skewed distribution while preserving all available minority-class examples.
    \item \textbf{Cosine Learning Rate Schedule}: A cosine annealing schedule with linear warmup (6\% of training steps) provides smooth decay, avoiding destructive updates in early training and premature convergence.
    \item \textbf{Gradient Accumulation}: Per-device batch size of 16 with 2 accumulation steps yields an effective batch size of 32, balancing GPU memory constraints (16\,GB VRAM on Kaggle T4) with gradient stability.
    \item \textbf{Early Stopping}: Validation Macro F1 is monitored with patience of 3 evaluation rounds to halt training before overfitting.
\end{itemize}

\paragraph{Note on planned approaches.} We also considered hierarchical classification (first binary human-vs-machine, then LLM family identification) and weighted random sampling via PyTorch's \texttt{WeightedRandomSampler}. These remain promising directions for future work but were not implemented in the current system due to the effectiveness of the class-weighted loss approach.

\paragraph{Applicability to Subtask~C.} While Subtask~C's class imbalance is less extreme than Subtask~B's (54\% human vs.\ 88\%), the hybrid class (9\%) and adversarial class (13\%) are underrepresented. We plan to transfer the class-weighted loss and label smoothing strategies developed here to Subtask~C in subsequent experiments, adapting the weighting scheme to the 4-class distribution.
...
Model diversity is key: CodeBERT captures token-level patterns, GraphCodeBERT captures data-flow structure, and UniXcoder leverages AST information---providing complementary views of the same code. The ensemble pipeline will be applied to all three subtasks, with task-specific weight tuning.
...
\begin{enumerate}
    ...
    \item It is robust to the severe class imbalance in Subtask~B and the moderate imbalance in Subtask~C.
\end{enumerate}
...
C & CodeBERT (baseline) & ---$^\dagger$ & ---$^\dagger$ \\
\bottomrule
\end{tabularx}
...
$^\dagger$Subtask~C baseline has been run using the official CodeBERT starter notebook with 10\% data subsampling. Full validation metrics are pending completion of the full-data training run.
\end{minipage}
...
Three classification head configurations are explored:
...
Ablation studies evaluating different combinations of the proposed features were conducted using a subset of 2,000 training samples and 500 validation and test samples due to computational constraints. This preliminary analysis enabled rapid assessment of the relative contribution of each feature, allowing us to identify beneficial components and discard detrimental ones. Consequently, these experiments informed the selection of the final model architecture while significantly reducing the computational cost of exhaustive experimentation.
...
\textbf{UniXcoder} consistently achieved worse results compared to CodeBERT base, suggesting the signals related to data flow and code structure captured are not AI vs.\ Human indicators. The task may often rely on surface cues. It may also have caused overfitting or have been the wrong model for this task.

\paragraph{BiLSTM heads hurt at this data scale.}
Both BiLSTM configurations underperform simpler counterparts (macro F1 drops from 0.42 to 0.35 for CodeBERT and 0.29 for UniXcoder). With only 2,000 training samples, the added parameters introduce optimization complexity that the transformer already handles internally, yielding redundancy without benefit.

\paragraph{Regularization outweighs architectural complexity.}
CodeBERT + smoothing matches the bare baseline ($\Delta \approx 0.00$) while the BiLSTM variant regresses.
...
\paragraph{Defense of the experimental setup.}
The 2,000-sample budget reflects genuine compute-limited constraints, not a shortcut, and cleanly isolates each design choice by fixing training set size across all six configurations. Macro-averaged F1 appropriately weights both classes equally under imbalance. Reporting per-class precision and F1 alongside aggregate metrics exposes the precision-recall trade-off that accuracy alone would obscure.
...
\midrule
CodeBERT (Baseline) & --- & --- \\
CodeBERT + Stylometry + Tuning (Final) & --- & --- \\
\bottomrule
\end{tabular}
...
\paragraph{Discussion.} \textit{To be filled in.}
...
Before developing improved models, we conducted a thorough audit of the provided Subtask~B baseline code. This revealed \textbf{six critical implementation bugs} that collectively rendered the baseline results misleading and significantly degraded model performance. Table~\ref{tab:taskB_bugs} catalogues each bug, its root cause, and its impact.
...
After fixing all six bugs, the corrected baseline CodeBERT model---still using standard cross-entropy loss with no class weighting---achieved Macro F1 $\approx$ 0.22 on the validation set. While this is an improvement over the silently broken version, the model still severely underperforms on minority classes due to the unaddressed class imbalance.
...
\subsubsection{Per-Class Analysis of Best Model}
\label{sec:taskB_perclass}

Table~\ref{tab:taskB_perclass} presents the per-class Precision, Recall, and F1 scores for the best hyperparameter configuration (Trial~4: CodeBERT, LR=$2{\times}10^{-5}$, max\_length=256, 2 epochs).
...
Human & 1.00 & 0.87 & 0.93 \\
DeepSeek-AI & 0.06 & 0.30 & 0.10 \\
Qwen & 0.19 & 0.14 & 0.16 \\
01-ai & 0.28 & 0.28 & 0.28 \\
BigCode & 0.41 & 0.58 & 0.48 \\
Gemma & 0.25 & 0.35 & 0.29 \\
Phi & 0.45 & 0.54 & 0.49 \\
Meta-LLaMA & 0.20 & 0.30 & 0.24 \\
IBM-Granite & 0.31 & 0.82 & 0.45 \\
Mistral & 0.07 & 0.18 & 0.10 \\
OpenAI & 0.43 & 0.63 & 0.51 \\ \midrule
\textbf{Macro Avg.} & \textbf{0.33} & \textbf{0.45} & \textbf{0.37} \\
\bottomrule
\end{tabularx}
\caption{Subtask~B per-class results (Trial~4: CodeBERT, LR=$2{\times}10^{-5}$, max\_length=256, 2 epochs).}
...
\begin{itemize}
    \item \textbf{Easily detected} (F1 $\geq$ 0.45): OpenAI (0.51), Phi (0.49), BigCode (0.48), and IBM-Granite (0.45). These families likely produce code with distinctive stylistic signatures---e.g., characteristic comment patterns, variable naming conventions, or structural idioms that differ from human code and other LLM families.
    \item \textbf{Moderately detected} (F1 0.24--0.35): Gemma (0.29), 01-ai (0.28), and Meta-LLaMA (0.24). These families are partially distinguishable but frequently confused with each other or with human code.
    \item \textbf{Poorly detected} (F1 $\leq$ 0.16): Qwen (0.16), DeepSeek-AI (0.10), and Mistral (0.10). These are the hardest classes. The extremely low precision (Mistral: 0.07, DeepSeek: 0.06) indicates severe false-positive contamination, likely because their code generation style closely mimics human patterns or overlaps heavily with other LLM families.
\end{itemize}

\paragraph{Precision--Recall imbalance.} IBM-Granite exhibits a notably asymmetric profile: high recall (0.82) but low precision (0.31), meaning the model over-predicts this class---many samples from other classes are incorrectly labeled as IBM-Granite. This may indicate that IBM-Granite's code style serves as a ``catch-all'' pattern for ambiguous LLM-generated samples. Conversely, the human class shows near-perfect precision (1.00) but lower recall (0.87), meaning 13\% of human-written code is misclassified as machine-generated.
...
UniXcoder + cls.\ wt.\ + sub-20K & 5.7 & \textbf{0.470} & +0.25 \\
CodeBERT + cls.\ wt.\ + sub-40K + LS & 2 & 0.368 & +0.15 \\
\bottomrule
\end{tabularx}
\caption{Subtask~B experimental progression. ``cls.\ wt.'' = sqrt-scaled class weights, ``sub-$N$K'' = stratified subsample with human capped at $N$K, ``LS'' = label smoothing ($\alpha = 0.1$). The UniXcoder experiment achieves the highest Macro F1 but trains for $2.85\times$ more epochs.}
\label{tab:taskB_comparison}
\end{table}

\paragraph{Comparing Experiments~1 and~2.}
A direct comparison between UniXcoder (0.470 at 5.7 epochs) and CodeBERT (0.368 at 2 epochs) requires accounting for the different training durations. Experiment~1 trained for nearly $3\times$ as many epochs as Experiment~2. At comparable epoch counts (epoch~2), the UniXcoder model achieved Macro F1 $\approx$ 0.385, which is close to CodeBERT's 0.368. This suggests that the backbone model choice (UniXcoder vs.\ CodeBERT) contributes modestly, while the \textit{training duration} is the dominant factor. The additional regularization in Experiment~2 (label smoothing + higher weight decay) reduces overfitting risk but may require more epochs to converge.
...
Our analysis reveals that the 10 LLM families fall into three distinct detectability tiers. OpenAI, Phi, BigCode, and IBM-Granite are relatively easy to detect (F1 $\geq$ 0.45), likely because these model families produce code with distinctive structural patterns---e.g., characteristic comment formatting, consistent variable naming conventions, or idiomatic code organization that differs from human code. In contrast, DeepSeek-AI and Mistral are extremely difficult (F1 = 0.10), with precision as low as 0.06--0.07, indicating that their code generation style is nearly indistinguishable from human code using token-level features alone.

\subsection{Precision--Recall Trade-offs}

The class-weighted loss creates interesting precision--recall trade-offs:
\begin{itemize}
    \item \textbf{Human class}: Near-perfect precision (1.00) but reduced recall (0.87)---the model successfully avoids labeling LLM code as human, but 13\% of human code is misclassified as machine-generated. This is expected given our stratified subsampling which reduces human prevalence.
    \item \textbf{IBM-Granite}: High recall (0.82) but low precision (0.31)---the model identifies most IBM-Granite samples correctly but also incorrectly labels many samples from other classes as IBM-Granite. This class appears to act as a ``catch-all'' for ambiguous LLM-generated code.
    \item \textbf{DeepSeek-AI and Mistral}: Both show low precision (0.06, 0.07) with moderate recall (0.30, 0.18)---the model attempts to identify these classes but generates many false positives, suggesting their stylistic features overlap heavily with other classes.
\end{itemize}

\subsection{Overfitting and Generalization Concerns}

The validation loss divergence observed in Experiment~1 (Section~\ref{sec:taskB_perclass}) raises concerns about generalization to unseen authors and languages. The model's high performance on the human class but poor performance on minority LLM classes suggests that it may be learning superficial frequency-based features rather than deep stylistic signatures. This is consistent with the well-known challenge of learning from highly imbalanced distributions, where minority-class features are underrepresented in the embedding space.
...
CodeBERT full training with best HP (Task~B) & In Progress \\
...
\begin{enumerate}
    ...
    \item Submitted baseline predictions to the Kaggle Phase~1 public leaderboard.
...
\begin{enumerate}
    \item Full training of CodeBERT with best hyperparameters for 5 epochs on Subtask~B (expected to match or exceed Experiment~1's 0.470 Macro F1).
...
\begin{enumerate}
    ...
    \item Generate final predictions for Kaggle Phase~2 private test set submission across all three subtasks.
\end{enumerate}
...
Jan 31 -- Feb 07 & \textbf{Setup \& Baseline}: Environment setup, data exploration, CodeBERT baseline reproduction (Tasks~A, B, C) and Kaggle submission. \\ \addlinespace[0.5ex]
Feb 08 -- Feb 22 & \textbf{Model Experimentation}: GraphCodeBERT fine-tuning for Tasks~A and B; initial imbalance handling; Task~C baseline bug fixes and full-data run. \\ \addlinespace[0.5ex]
Feb 23 -- Mar 10 & \textbf{Head Engineering}: Implementing Bi-LSTM and dense classification heads; UniXcoder experiments; Task~C improved models with class-weighted loss and backbone experiments. \\ \addlinespace[0.5ex]
Mar 11 -- Mar 25 & \textbf{Ensembling \& Tuning}: Ensemble pipeline development across all three subtasks; threshold tuning; error analysis including Task~C adversarial--human confusion. \\ \addlinespace[0.5ex]
Mar 26 -- Apr 08 & \textbf{Finalization}: Full evaluation on private test set for Tasks~A, B, and~C; ablation study; report writing and presentation. \\ \bottomrule
\end{tabularx}
...
Our best result---Macro F1 of 0.470 with UniXcoder and class-weighted training---represents a $>$2$\times$ improvement over the corrected baseline ($\sim$0.22). CodeBERT with additional regularization achieves 0.368 in only 2 epochs, with significant headroom for further training. Per-class analysis reveals that OpenAI, Phi, BigCode, and IBM-Granite are the most detectable LLM families (F1 $\geq$ 0.45), while DeepSeek-AI and Mistral remain extremely challenging (F1 = 0.10).

For Subtask~A, ablation studies across backbone models (CodeBERT, UniXcoder) and classification heads (linear, Bi-LSTM, dense with stylometric features) were conducted, finding that the CodeBERT baseline with a linear head remains competitive on the small-scale evaluation.

For Subtask~C (4-class hybrid code detection), we have reproduced the official CodeBERT baseline, establishing reference performance and identifying the shared baseline bugs that need fixing. The class-imbalance handling, backbone experiments, and classification head improvements developed for Subtasks~A and~B provide a ready-made toolkit for improving Task~C performance, which is our immediate next step.

Remaining work includes full-epoch training with the best hyperparameters, GraphCodeBERT experiments to leverage data-flow features, improved Task~C models with class-weighted loss and backbone experiments, ensemble strategies across diverse models for all three subtasks, and comprehensive evaluation on unseen languages and domains.
